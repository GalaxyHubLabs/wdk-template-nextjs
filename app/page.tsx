    ...
    <main className="flex flex-1 flex-col">
      ...
      <section className="relative overflow-hidden px-6 pt-20 pb-16 sm:pt-28 sm:pb-24">
        ...
        <div className="mx-auto w-full max-w-3xl space-y-8 text-center">
          {/* GitHub badge — clickable. Visitors who land on the demo
              and want to fork get the repo URL in one click, and the
              badge format signals "this is a template you build on,
              not just a finished wallet". Plain-link styling so the
              repo path reads as the source-of-truth itself. */}
          <a
            href="https://github.com/GalaxyHubLabs/wdk-template-nextjs"
            target="_blank"
            rel="noreferrer"
            className="group inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white px-3 py-1 text-xs font-medium text-zinc-700 transition-colors hover:border-brand/40 hover:text-foreground dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300"
          >
            <GithubMark size={12} />
            <span className="font-mono text-zinc-500 group-hover:text-foreground dark:text-zinc-400">
              GalaxyHubLabs/wdk-template-nextjs
            </span>
            <span aria-hidden className="text-zinc-300 dark:text-zinc-700">
              ·
            </span>
            <span>Fork the template</span>
            <ArrowRight
              size={11}
 ...
 */
/**
 * Inline GitHub mark. lucide-react doesn't ship a Github icon — we
 * inline the canonical octocat path so the GitHub badge in the hero
 * doesn't depend on an additional icon library.
 */
